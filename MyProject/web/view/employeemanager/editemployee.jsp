<%-- 
    ...
    Created on : Nov 3, 2024, 11:59:07 PM
    Author     : ADMIN
--%>
...
<div class="container">
    ...
    <form action="editemployee" method="POST">
        ...
        <input type="text" id="salaryLevel" name="salaryLevel" value="${employee.salaryLevel}" required />

        <label for="did">Phòng Ban:</label>
        <select id="did" name="did" required>
            <c:forEach var="dept" items="${departments}">
                <option value="${dept.id}" <c:if test="${dept.id == employee.did}">selected</c:if>>${dept.name}</option>
            </c:forEach>
        </select>

        <label for="createdBy">Người Tạo:</label>
        <input type="text" id="createdBy" name="createdBy" value="${employee.createdBy}" required />
